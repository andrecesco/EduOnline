...
public class Validacoes
{
    ...
    {
        if (!valor.HasValue || valor < minimo || valor > maximo)
        {
            throw new DomainException(mensagem);
        }
    }
    ...
    public static void ValidarSeMenorQue(DateOnly valor, DateOnly minimo, string mensagem)
    {
        if (valor < minimo)
        {
            throw new DomainException(mensagem);
    ...
    public static void ValidarSeMaiorQue(DateOnly valor, DateOnly maximo, string mensagem)
    {
        if (valor > maximo)
        {
            throw new DomainException(mensagem);
    ...
    {
        if (!valor.HasValue || valor < minimo)
        {
            throw new DomainException(mensagem);
        }
    }
    ...
    public static void ValidarEnumerador<T>(int? id, string mensagem) where T : Enumerador
    {
        if (!id.HasValue || Enumerador.GetById<T>(id.Value) == null)
        {
            throw new DomainException(mensagem);
        }
    }
